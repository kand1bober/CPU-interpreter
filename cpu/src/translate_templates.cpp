...
#define SIGN_EXTEND(reg1, reg2, imm) \
    as.mov(reg2, imm); \
    as.movsx(reg1, reg2);
...
void translate_kSt(Assembler& as, CpuState* cpu_state, Memory* memory, uint8_t base, uint8_t rt, int16_t offset)
{
    ...
    as.mov(ebx, dword_ptr(rdi, OFF(base)));
    as.movzx(rbx, ebx);
    as.add(rax, rbx); //add base

    SIGN_EXTEND(rbx, bx, offset)
    as.add(rax, rbx); //add offset

    as.mov(ebx, dword_ptr(rdi, OFF(rt)));    
    as.mov(dword_ptr(rax), ebx);    
}
...
void translate_kLdp(Assembler& as, CpuState* cpu_state, Memory* memory, uint8_t base, uint8_t rt1, uint8_t rt2, int16_t offset)
{
    ...
    as.mov(dword_ptr(rdi, OFF(rt2)), ebx); //ld rt2    
}
